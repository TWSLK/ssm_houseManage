...
function checkAll() {
	...
	var tel = $("#txtPhone").val();
	/*
	 * var name1=$("#txtName1").val(); var name2=$("#txtName2").val();
	 */
	var regtel = /^(13|14|15|16|17|18|19)\d{9}$/;
...
function checkMiBao() {
	...
	if (mbquestion != question) {
		...
		/*
		 * $("#checku").html(""); $.ajax({ url : "checkMiBao.do", type : "post",
		 * data : "uname=" + name, dataType : "json", success : function(result) {
		 * if (result.info == "ng") { $("#checkUname").html("用户名不存在，请重新输入！");
		 * $("input[id=uname_loss]").focus(); return false; } else {
		 * $("#checkUname").html("用户名正确！");
		 * $("input[id=mbanswer_loss]").val(result.info);
		 * $("#showMbanswer").show(); $("#showQuestion").show(); return true; } },
		 * error : function() { alert("请求失败！"); return false; }
		 * 
		 * });
		 */
	}
